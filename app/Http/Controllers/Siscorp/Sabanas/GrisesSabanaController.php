...
class GrisesSabanaController extends Controller
{
    public function seleccionaEmpresa($tipo_control){
        return view('sabanas.Empresa.busca')->with('tipo_control',$tipo_control)->with('ruta','sabanas.buscar.empresa.gris');
    }
    ...
    public function seleccionaCiudadano($tipo_control){
        return view('sabanas.Ciudadano.busca')->with('tipo_control',$tipo_control)->with('ruta','sabanas.buscar.ciudadano.gris');
    }
    ...
    public function buscaPorCiudadano( Request $request, SicoveApiConsume $sicove ){
        ...
        if( !$respuesta || (isset($respuesta['data']) && $respuesta['data'] == false) ){
            return view('sabanas.Ciudadano.busca')->with('tipo_control',$request->tipo_control)->with('ruta','sabanas.buscar.ciudadano.gris')
                ->withErrors('No se encontraron resultados con los parametros de busqueda');
        }
        $controles = $sicove->getListaControles();
        $res = $this->filtroActivas($respuesta['data']);
    ...
    public function seleccionaVehiculo($tipo_control){
        return view('sabanas.Vehiculo.busca')->with('tipo_control',$tipo_control)->with('ruta','sabanas.buscar.vehiculo.gris');
    }
    ...
    public function buscaPorVehiculo(Request $request, SicoveApiConsume $sicove){
        ...
        if( !$respuesta || (isset($respuesta['data']) && $respuesta['data'] == false) ){
            return view('sabanas.Vehiculo.busca')->with('tipo_control',$request->tipo_control)->with('ruta','sabanas.buscar.vehiculo.gris')
                ->withErrors('No se encontraron resultados con los parametros de busqueda');
        }
        $controles = $sicove->getListaControles();
        $res = $this->filtroActivas($respuesta['data']);
...
Folio Identificador: '.$datos['linea_captura'].'_'.now()->getTimestamp();
        ...
        $datos['QR'] = $qrcode;
        $datos['fecha_expedicion_sabana'] = date('Y-m-d');

        $pdf = PDF::loadView($layout, compact('datos'));

        $nombre_pdf = 'Reporte_SISCORP_'.$datos['placa'].'.pdf';
        $this->logActividad($user->id, 'GENERA SABANA', 'CONSULTA WS CON LC'.$lc, $user->direccion_id);
